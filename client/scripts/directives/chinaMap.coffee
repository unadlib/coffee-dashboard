do ->
    'use strict'
    angular.module('ChinaMap', []).directive('showmap', [
        '$document'
        'ChinaMap'
        'SVGMap'
        ($document, ChinaMap, SVGMap) ->
            {
                scope: data: '='
                template:
                    """
                        ...
                        <style>
                            ...
                            .regionList ul{float:left;width:110px;height:100%;margin-right:5px;display:inline; padding: 0;}
                            .regionList ul li{height:24px;padding:0 0px;border:1px solid #fff;line-height:24px;list-style:none;}
                            .regionList ul li.select{border:1px solid #D3D3D3;background:#FFF1BF}
                            .mapInfo i{display:inline-block;width:15px;height:15px;margin-top:5px;line-height:15px;font-style:normal;background:#aeaeae;color:#fff;font-size:11px;font-family:Tahoma;-webkit-border-radius:15px;border-radius:15px;text-align:center}
                            .mapInfo i.active{background:#E27F21;}
                            .mapInfo span{padding:0 5px 0 3px;}
                            .mapInfo b{font-weight:normal;color:#2770B5}
                            .regionMap{float:left;display:inline;}
                        </style>
                    """
                link: (scope, $element, $attr) ->
                    scope.$watch 'data.stateData', ((newvalue, oldvalue) ->
                        if angular.equals(newvalue, oldvalue)
                            return false
                        new SVGMap($element, scope.data)
                        return
                    ), false
                    return
        ...
        ($http, ChinaMap) ->
            do ->
                ...
                SVGMap::options =
                    ...
                    stateTipX: 0
                    stateTipY: -10
                    stateTipHtml: (mapData, obj) ->
                        if typeof mapData[obj.id] == 'undefined'
                            return false
                        _value = mapData[obj.id].value
                        _idx = mapData[obj.id].index
                        _percent = mapData[obj.id].percent
                        active = ''
                        if _idx < 4 then (active = 'active') else (active = '')
                        tipStr = '<div class="mapInfo"><i class="' + active + '">' + _idx + '</i><span>' + obj.name + '</span><b>' + _value + '</b></div>'
                        tipStr
                    hoverCallback: (stateData, obj) ->
                    clickCallback: (stateData, obj) ->
                    external: {}

                SVGMap::setOptions = (options) ->
                    for key of options
                        @options[key] = options[key]
                    @options

                SVGMap::scaleRaphael = (container, width, height) ->
                    wrapper = document.getElementsByClassName(container)[0]
                    if !wrapper.style.position
                        wrapper.style.position = 'relative'
                    wrapper.style.width = width + 'px'
                    wrapper.style.height = height + 'px'
                    wrapper.style.overflow = 'hidden'
                    nestedWrapper = undefined
                    if Raphael.type == 'VML'
                    ...
                    paper.changeSize = (w, h, center, clipping) ->
                        clipping = !clipping
                        ratioW = w / width
                        ratioH = h / height
                        scale = if ratioW < ratioH then ratioW else ratioH
                        newHeight = parseInt(height * scale)
                        newWidth = parseInt(width * scale)
                        if Raphael.type == 'VML'
                            txt = document.getElementsByTagName('textpath')
                            for i of txt
                                curr = txt[i]
                                if curr.style
                                    if !curr._fontSize
                                        mod = curr.style.font.split('px')
                ...
                SVGMap::getNameByZh = (zh_name) ->
                    for i of ChinaMap.names
                        if ChinaMap.names[i] == zh_name
                            return i
                    'Unknow'
                ...
                SVGMap::flashData = ->
                    ...
                    total = 0
                    for key of stateData
                        stateData[key]['dp'] = @getNameByZh(stateData[key].name)
                        if stateData[key]['dp'] == 'Unknow'
                            delete stateData[key]
                            continue
                        total += parseInt(stateData[key].value)
                    stateData.sort (a, b) ->
                        a.value - (b.value)
                    stateData.reverse()
                    tmp = {}
                    for tmpsKey of stateData
                        tmp[stateData[tmpsKey]['dp']] = stateData[tmpsKey]
                        tmp[stateData[tmpsKey]['dp']].index = parseInt(tmpsKey) + 1
                        tmp[stateData[tmpsKey]['dp']].percent = Math.floor(tmp[stateData[tmpsKey]['dp']].value * 100 / total) + '%'
                        tmp[stateData[tmpsKey]['dp']].stateInitColor = 4
                        if tmpsKey <= 8
                            tmp[stateData[tmpsKey]['dp']].stateInitColor = 0
                        if tmpsKey > 8 and tmpsKey <= 16
                            tmp[stateData[tmpsKey]['dp']].stateInitColor = 1
                        if tmpsKey > 16 and tmpsKey <= 300
                            tmp[stateData[tmpsKey]['dp']].stateInitColor = 2
                    ...
                    if opt.showAll == true
                        for i of ChinaMap.names
                            if typeof tmp[i] == 'undefined'
                                key++
                                tmp[i] =
                                    name: ChinaMap.names[i]
                                    index: key
                                    value: 0
                                    percent: '0%'
                                    stateInitColor: 4
                    @options.stateData = tmp
                    return
                ...
                SVGMap::render = ->
                    ...
                    if opt.showlist
                        i = 1
                        @parentDom.find('.regionList .list1').empty()
                        @parentDom.find('.regionList .list2').empty()
                        @parentDom.find('.regionList .list3').empty()
                        for k of stateData
                            if i <= 12
                                _cls = if i < 4 then 'active' else ''
                                @parentDom.find('.regionList .list1').append '<li name="' + k + '"><div class="mapInfo"><i class="' + _cls + '">' + i++ + '</i><span>' + ChinaMap.names[k] + '</span><b>' + stateData[k].value + '</b></div></li>'
                            else if i <= 24
                                @parentDom.find('.regionList .list2').append '<li name="' + k + '"><div class="mapInfo"><i>' + i++ + '</i><span>' + ChinaMap.names[k] + '</span><b>' + stateData[k].value + '</b></div></li>'
                            else
                                @parentDom.find('.regionList .list3').append '<li name="' + k + '"><div class="mapInfo"><i>' + i++ + '</i><span>' + ChinaMap.names[k] + '</span><b>' + stateData[k].value + '</b></div></li>'
                    ...
                    offsetXY = (e) ->
                        ...
                        mouseX = if mouseX - (tipWidth / 2) + opt.stateTipX < 0 then 0 else mouseX - (tipWidth / 2) + opt.stateTipX
                        mouseY = if mouseY - tipHeight + opt.stateTipY < 0 then mouseY - (opt.stateTipY) else mouseY - tipHeight + opt.stateTipY
                        [
                            mouseX
                            mouseY
                        ]
                    ...
                    current = undefined
                    reTimer = undefined
                    r = @scaleRaphael(_self.attr('class'), mapConfig.width, mapConfig.height)
                    attributes =
                    ...
                    stateColor = {}
                    for state of mapConfig.shapes
                        thisStateData = stateData[state]
                        initColor = '#' + (thisStateData and opt.stateColorList[thisStateData.stateInitColor] or opt.stateInitColor)
                        hoverColor = '#' + (thisStateData and thisStateData.stateHoverColor or opt.stateHoverColor)
                        selectedColor = '#' + (thisStateData and thisStateData.stateSelectedColor or opt.stateSelectedColor)
                        disabledColor = '#' + (thisStateData and thisStateData.stateDisabledColor or opt.stateDisabledColor)
                        stateColor[state] = {}
                        stateColor[state].initColor = initColor
                        stateColor[state].hoverColor = hoverColor
                        stateColor[state].selectedColor = selectedColor
                        obj = r.path(mapConfig['shapes'][state])
                        obj.id = state
                        obj.name = mapConfig['names'][state]
                        obj.attr attributes
                        if opt.external
                            opt.external[obj.id] = obj
                        if stateData[state] and stateData[state].diabled
                            obj.attr
                                fill: disabledColor
                                cursor: 'default'
                        else
                            ...
                            obj.hover (e) ->
                                if this != current
                                    @animate { fill: stateColor[@id].hoverColor }, 250
                                if opt.showTip
                                    clearTimeout reTimer
                                    if $('.stateTip').length == 0
                                        $(document.body).append '<div class="stateTip"></div'
                                    thisData = opt.stateTipHtml(stateData, this)
                                    if thisData != false
                                        $('.stateTip').html thisData
                                        _offsetXY = new offsetXY(e)
                                        $('.stateTip').css(
                                            width: opt.stateTipWidth or 'auto'
                                            height: opt.stateTipHeight or 'auto'
                                            left: _offsetXY[0]
                                            top: _offsetXY[1]).show()
                                opt.hoverCallback stateData, this
                                return
                            obj.mouseout ->
                                if this != current
                                    @animate { fill: stateColor[@id].initColor }, 250
                                # $('.stateTip').hide();
                                if opt.showTip
                                    reTimer = setTimeout((->
                                        $('.stateTip').remove()
                            ...
                            obj.mouseup (e) ->
                                if current
                                    current.animate { fill: stateColor[current.id].initColor }, 250
                                @animate { fill: stateColor[@id].selectedColor }, 250
                                current = this
                    ...
                    if opt.showlist
                        _external = opt.external
                        _parentDom = @parentDom
                        _stateColorList = @options.stateColorList
                        _data = @data
                        _parentDom.find('.regionList li').hover (->
                            thisName = $(this).attr('name')
                            thisHtml = $(this).html()
                            _parentDom.find('.regionList li').removeClass 'select'
                            $(this).addClass 'select'
                            $(document.body).append '<div id="StateTip"></div'
                            $('#StateTip').css(
                                left: $(_external[thisName].node).offset().left - 50
                                top: $(_external[thisName].node).offset().top - 40).html(thisHtml).show()
                            _external[thisName].attr fill: '#E99A4D'
                            return
                        ), ->
                            thisName = $(this).attr('name')
                            $('#StateTip').remove()
                            _parentDom.find('.regionList li').removeClass 'select'
                            _external[$(this).attr('name')].attr fill: '#' + _stateColorList[_data[$(this).attr('name')].stateInitColor]
                            return
                    else
                        @parentDom.find('.regionList').remove()
                    return
